defmodule Scrivener.Phoenix.RoutesTestBis do
  ...
  defp do_test(fun, conn, expected)
  ...
  do
    ...
    for {conn, route, args} <- fun.(conn) do
      ...
      assert fun.(2) == expected
    end
  end

  describe "XXX" do
    test "blog_post_path", %{conn: conn} do
      do_test(&blog_post_path/1, conn, "/blog/posts?page=2")
    end

    test "blog_post_url", %{conn: conn} do
      do_test(&blog_post_url/1, conn, "#{@url}/blog/posts?page=2")
    end

    test "blog_seite_path", %{conn: conn} do
      do_test(&blog_seite_path/1, conn, "/blog/posts/seite/2")
    end

    test "blog_seite_url", %{conn: conn} do
      do_test(&blog_seite_url/1, conn, "#{@url}/blog/posts/seite/2")
    end

    test "forum_topic_path", %{conn: conn} do
      do_test(&forum_topic_path/1, conn, "/forum/topics/643?page=2")
    end

    test "forum_topic_url", %{conn: conn} do
      do_test(&forum_topic_url/1, conn, "#{@url}/forum/topics/587?page=2")
    end

    test "forum_topic_page_path", %{conn: conn} do
      do_test(&forum_topic_page_path/1, conn, "/forum/topics/127/page/2")
    end

    test "forum_topic_page_url", %{conn: conn} do
      do_test(&forum_topic_page_url/1, conn, "#{@url}/forum/topics/946/page/2")
    end
  end
end
